...
use candle_core::{utils::{cuda_is_available, metal_is_available}, DType, Device, IndexOp, Module, Tensor, D};
pub use clap::Parser;
...
/// (c, height, width).
pub fn save_image_inner<P: AsRef<std::path::Path>>(img: &Tensor, p: P) -> Result<()> {
    let p = p.as_ref();
    ...
    let image: image::ImageBuffer<image::Rgb<u8>, Vec<u8>> =
        match image::ImageBuffer::from_raw(width as u32, height as u32, pixels) {
            Some(image) => image,
            None => todo!()//candle_core::bail!("error saving image {p:?}"),
        };
...
fn save_image(
    ...
    for batch in 0..bsize {
        ...
        save_image_inner(&image, image_filename)?;
    }
    Ok(())
}
